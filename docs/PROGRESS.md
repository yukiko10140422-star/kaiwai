...
### 2026-03-10 セッション6
**実施内容:**
- Adobe風ファイルプレビューモーダル改善 (v1.11.4)
  - `useZoomPan` カスタムフック作成（ホイールズーム・ドラッグパン・ピンチズーム・ダブルクリックトグル）
  - `ViewerToolbar` フローティングツールバー作成（3秒自動フェードアウト、ズーム/フィット/ページ送り/ダウンロード/詳細）
  - `FilePreviewModal` を useZoomPan + ViewerToolbar に全面改修
  - `PdfPreview` のページナビゲーションを ViewerToolbar に移譲
  - 「幅に合わせる」「全体表示」フィットプリセット追加
  - モバイルでボトムナビと重ならないよう配慮
