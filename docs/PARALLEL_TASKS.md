...
#### Task W: タスクコメントUI ⏳
**優先度:** 🔴 最優先 | **難易度:** 低
**内容:** タスク詳細モーダルでコメント・議論ができるUI。API（`createTaskComment`, `fetchTaskComments`）は実装済み。
**参照ファイル:**
- `src/lib/tasks.ts` - `fetchTaskComments`, `createTaskComment`（既に実装済み）
- `src/components/tasks/TaskDetailModal.tsx` - ここにコメントセクションを追加
- `src/types/database.ts` - `TaskComment` 型（既に定義済み）
**依存:** なし

#### Task X: ダークモード配色調整 ⏳
**優先度:** 🔴 最優先 | **難易度:** 低
**内容:** チャットバブル、モーダル、カードの配色をダーク/ライトで統一感ある調整
**参照ファイル:**
- `src/app/globals.css` - テーマ変数定義
- 各コンポーネント - 配色確認
**依存:** なし

---

### 🟡 バッチ2: 次にやる（中難易度・高インパクト）

#### Task Y: タスク期限通知
**優先度:** 🟡 高 | **難易度:** 中
**内容:** 期限が近い/過ぎたタスクの通知を自動送信。ダッシュボードログイン時にチェック。
**参照ファイル:**
- `src/lib/notifications.ts` - 通知作成関数
- `src/lib/tasks.ts` - タスク取得
- `src/types/database.ts` - `NotificationType` に `task_due` あり
**依存:** なし

#### Task Z: ピン留めメッセージ
**優先度:** 🟡 高 | **難易度:** 中
**内容:** 重要なメッセージをチャンネル上部に固定表示
**参照ファイル:**
- `src/lib/chat.ts` - メッセージ関連関数
- `src/components/chat/ChannelHeader.tsx` - ピン留めメッセージ表示エリア
- DB: `messages` テーブルに `is_pinned` カラム追加が必要
**依存:** DB マイグレーション

#### Task AA: メッセージ既読表示
**優先度:** 🟡 高 | **難易度:** 中
**内容:** LINE風の「既読」表示（誰が読んだか）
**参照ファイル:**
- `src/lib/chat.ts` - `updateReadStatus` 関数（既存）
- `src/components/chat/MessageItem.tsx` - 既読表示UI
- DB: `channel_read_status` テーブル（既存）を活用
**依存:** なし

#### Task AB: アクティビティログUI
**優先度:** 🟡 高 | **難易度:** 中
**内容:** 「誰がいつ何をした」の履歴タイムライン表示
**参照ファイル:**
- `src/types/database.ts` - `ActivityLog`, `ActivityAction` 型（既に定義済み）
- DB: `activity_logs` テーブル（既に作成済み）
- `src/app/dashboard/progress/page.tsx` - ダッシュボードに統合
**依存:** なし

---

### 🟢 バッチ3: その次（中難易度・中インパクト）

#### Task AC: ブラウザプッシュ通知
**優先度:** 🟢 中 | **難易度:** 中
**内容:** アプリを開いてなくても通知が届くブラウザ通知API
**依存:** なし

#### Task AD: KPI/目標管理UI
**優先度:** 🟢 中 | **難易度:** 中
**内容:** プロジェクトごとのKPI設定と進捗追跡（`kpi_goals` テーブルは既にDB作成済み）
**依存:** なし

#### Task AE: 週次レポート自動生成
**優先度:** 🟢 中 | **難易度:** 中
**内容:** その週の完了タスク・議事録・アクティビティをまとめて表示
**依存:** Task AB（アクティビティログ）
...
#### Task AG: Googleカレンダー連携
**優先度:** 🔵 低 | **難易度:** 高
**内容:** タスクの期限をGoogleカレンダーに同期（OAuth連携が必要）

#### Task AH: オフライン対応
**優先度:** 🔵 低 | **難易度:** 高
**内容:** Service Workerで最低限のオフライン閲覧
